...
class DataDumper(bl2sdk.BL2MOD):
    ...
    def Enable(self):
        ...
        for i in range(iterations):
            getall_file = 'dumper-getall.{}'.format(i)
            bl2sdk.Log('Writing exec file "{}"'.format(getall_file))
            self.getall_files.append(getall_file)
            with open(os.path.join(binaries_path, getall_file), 'w') as df:
        ...
        objdump_dir = os.path.join(binaries_path, 'objdump')
        ...
        for i in range(iterations):
            ...
            with open(os.path.join(objdump_dir, defaults_filename), 'w') as df:
        ...
        self.objdump_files = {}
        for filename in sorted(os.listdir(objdump_dir)):
            try:
                (levelname, num) = filename.split('.', 1)
                if levelname not in self.objdump_files:
                    self.objdump_files[levelname] = []
                self.objdump_files[levelname].append('objdump/{}'.format(filename))
            except ValueError:
                pass
